...
def main():
    ...
    config_path = os.path.join(
        os.path.dirname(__file__), "..", ".config_examples", "openai.yaml"
    )
    ...
    print("\nCache:")
    print(
        f"  Embedding hit rate: {summary['cache']['embedding_hit_rate_percent']:.2f}%"
    )
    ...
    if tool_def_hit_rate >= 95:
        ...
        print(
            f"⚠ Tool definition cache could be improved ({tool_def_hit_rate:.1f}% < 95%)"
        )
        print("  Consider:")
