...
def example_basic_multi_agent():
    ...
    result = coordinator.execute_collaborative_task(
        goal="Design and implement a simple REST API for a todo list", max_iterations=10
    )
...
def example_software_dev_team():
    ...
    for agent_id, (role, skills) in team.items():
        agent = CollaborativeAgent(
            ...
            llm=llm,
            capability=AgentCapability(
                role=role, skills=skills, max_concurrent_tasks=1
            ),
        )
...
def example_debugging_learning():
    ...
    recommendations = learning.get_recommendations(
        "Fix memory leak in application", context={"bug_category": "memory_leak"}
    )
...
def example_combined_multi_agent_learning():
    ...
    for task_desc in tasks:
        ...
        learning.record_experience(
            ...
            context={
                "agent_roles": [
                    a.capability.role.value for a in coordinator.agents.values()
                ],
...
if __name__ == "__main__":
    ...
    try:
        ...
        for db_file in [
            "example_learning.db",
            "debugging_learning.db",
            "collaborative_learning.db",
            "agent_a.db",
            "agent_b.db",
        ]:
