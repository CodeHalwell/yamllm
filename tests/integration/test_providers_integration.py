...
class TestProvidersIntegration:
    ...
    @pytest.mark.parametrize("provider_name,config", PROVIDER_CONFIGS.items())
    def test_provider_streaming_completion(
        self, provider_name: str, config: Dict[str, Any]
    ):
...
class TestAsyncProvidersIntegration:
    ...
    async def test_async_provider_basic_completion(
        self, provider_name: str, config: Dict[str, Any]
    ):
    ...
    async def test_async_provider_streaming(
        self, provider_name: str, config: Dict[str, Any]
    ):
    ...
    async def test_concurrent_async_requests(self):
        ...
        async def query_provider(provider_name: str, config: Dict[str, Any]):
            ...
            async with provider:
                response = await provider.get_completion(
                    messages=[
                        {"role": "user", "content": f"Say '{provider_name} works!'"}
                    ],
                    model=config["model"],
                    temperature=0.5,
                    max_tokens=50,
        ...
        tasks = [query_provider(name, config) for name, config in providers_to_test]
...
class TestProviderPerformance:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            for name, conf in PROVIDER_CONFIGS.items()
            if ProviderFactory.supports_async(name)
            and name == "openai"  # Test with one provider
        ],
    )
