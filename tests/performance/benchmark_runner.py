...
class BenchmarkRunner:
    ...
    def benchmark_metrics_overhead(self):
        ...
        print(
            f"  Status: {'✓ PASS' if overhead_per_call < 10 else '✗ FAIL'} (target: <10µs)"
        )
    ...
    def generate_report(self) -> str:
        ...
        # Overall status
        all_passed = all(result.get("pass", False) for result in self.results.values())
...
def main():
    ...
    all_passed = all(result.get("pass", False) for result in runner.results.values())
