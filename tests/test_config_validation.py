...
def test_mask_sensitive_data_masks_keys():
    ...
    assert masked["nested"]["token"].startswith("tok_") and masked["nested"][
        "token"
    ].endswith("6789")
...
def test_validator_flags_api_key_in_config():
    cfg = {
        ...
        "request": {
            "timeout": 5,
            "retry": {"max_attempts": 1, "initial_delay": 1, "backoff_factor": 1},
        },
        "context": {
            "system_prompt": "x",
    ...
    assert any(
        "API keys must not be stored in configuration files" in e for e in errors
    )
