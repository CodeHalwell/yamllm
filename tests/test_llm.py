...
def mock_llm():
    ...
    with patch("yamllm.core.llm.LLM.load_config", return_value=mock_config):
        llm = LLM(
            ...
            api_key=os.environ.get("OPENAI_API_KEY", "test-fake-key"),
        )
...
def test_query_missing_api_key():
    ...
    with patch("yamllm.core.llm.LLM.load_config", return_value=mock_config):
        ...
        with patch("openai.OpenAI"):
            ...
            with pytest.raises(
                ValueError, match="API key is not initialized or invalid."
            ):
...
def test_query_unexpected_error(mock_llm):
    ...
    with pytest.raises(
        Exception, match="Unexpected error during query: Unexpected error"
    ):
        mock_llm.query("Test prompt")
    ...
    with patch("yamllm.core.llm.LLM.load_config", return_value=mock_config):
        with patch("openai.OpenAI"), patch("yamllm.core.llm.time.sleep") as mock_sleep:
            ...
            mock_sleep.assert_called_once_with(
                0.01
            )  # initial_delay * (backoff_factor ^ 0)
