...
class TestResponseModels:
    ...
    def test_completion_response_basic(self):
        ...
        response = CompletionResponse(
            content="Hello world", model="test-model", provider="test"
        )
    ...
    def test_completion_response_with_tools(self):
        ...
        tool_call = ToolCall(
            id="call_123",
            type="function",
            function=ToolFunction(name="test_function", arguments='{"param": "value"}'),
        )

        response = CompletionResponse(
            content="Using tool...", tool_calls=[tool_call], provider="test"
        )
...
class TestToolExceptionHandling:
    ...
    def test_tool_manager_generic_error(self):
        ...
        tool = MockTool(
            should_fail=True, fail_with=RuntimeError("Something went wrong")
        )
