...
def run_chat(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if api_key_env:
            ...
            if not api_key:
                console.print(
                    f"[red]✗ Environment variable {api_key_env} not set[/red]"
                )
                return 1
        else:
            # Try common env vars
        ...
        if not api_key:
            ...
            console.print(
                "[dim]Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or GOOGLE_API_KEY[/dim]"
            )
            console.print(
                "[dim]Or use --api-key-env to specify a custom environment variable[/dim]"
            )
            return 1
...
def setup_chat_commands(subparsers):
    ...
    chat.add_argument(
        "--api-key-env",
        default=None,
        help="Env var name holding provider API key (e.g., OPENAI_API_KEY)",
    )
    chat.add_argument(
        "--style", default="bubble", help="Chat UI style: bubble|minimal|compact"
    )
    chat.add_argument(
        "--enhanced",
        action="store_true",
        help="Use enhanced chat interface with commands and shortcuts",
    )
