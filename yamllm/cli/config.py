...
def create_config(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        console.print(
            "[dim]Remember to set your API key in environment variables.[/dim]"
        )
...
def setup_config_commands(subparsers):
    ...
    create_cmd.add_argument(
        "--preset",
        default="casual",
        choices=["casual", "coding", "research", "minimal"],
    ...
    create_cmd.add_argument(
        "--max-tokens", type=int, default=1000, help="Maximum tokens"
    )
    create_cmd.add_argument(
        "--streaming", action="store_true", default=True, help="Enable streaming"
    )
    create_cmd.add_argument(
        "--no-streaming",
        action="store_false",
    ...
    # Config presets
    presets_cmd = config_sub.add_parser(
        "presets", help="List available configuration presets"
    )
