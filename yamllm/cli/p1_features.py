...
def setup_p1_commands(subparsers):
    ...
    tool_parser = subparsers.add_parser(
        "tool", help="Dynamic tool creation and management"
    )
    ...
    create_parser = tool_subparsers.add_parser(
        "create", help="Create tool from natural language description"
    )
    create_parser.add_argument("description", help="Tool description")
    create_parser.add_argument(
        "--name", help="Tool name (auto-generated if not provided)"
    )
    ...
    code_parser = subparsers.add_parser(
        "code", help="Code context intelligence and analysis"
    )
    code_subparsers = code_parser.add_subparsers(
        dest="code_command", help="Code commands"
    )

    # yamllm code analyze
    analyze_parser = code_subparsers.add_parser(
        "analyze", help="Analyze code repository"
    )
    ...
    context_parser = code_subparsers.add_parser(
        "context", help="Extract relevant code context for query"
    )
    ...
    context_parser.add_argument(
        "--max-symbols", type=int, default=10, help="Max symbols to include"
    )
    context_parser.add_argument("--config", help="LLM config")
    context_parser.set_defaults(func=get_code_context)

    # yamllm git
    git_parser = subparsers.add_parser("git", help="Advanced git workflow automation")
    ...
    smart_commit_parser = git_subparsers.add_parser(
        "smart-commit", help="Create commit with AI-generated message"
    )
    smart_commit_parser.add_argument("--repo", default=".", help="Repository path")
    smart_commit_parser.add_argument(
        "--files", nargs="+", help="Specific files to commit"
    )
    smart_commit_parser.add_argument(
        "--message", help="Override auto-generated message"
    )
    ...
    smart_branch_parser = git_subparsers.add_parser(
        "smart-branch", help="Create intelligently named branch"
    )
    ...
    smart_branch_parser.add_argument(
        "--strategy",
        choices=["gitflow", "github_flow", "trunk_based"],
        default="github_flow",
    ...
    auto_pr_parser = git_subparsers.add_parser(
        "auto-pr", help="Generate PR title and description"
    )
    ...
    # yamllm git status
    git_status_parser = git_subparsers.add_parser("status", help="Enhanced git status")
...
def analyze_code(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        top_files = sorted(
            context.files.items(), key=lambda x: x[1].complexity_score, reverse=True
        )[:5]
...
def get_code_context(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        context_text = intel.get_relevant_context(
            args.query, max_symbols=args.max_symbols
        )

        console.print(
            Panel(context_text, title="Relevant Context", border_style="cyan")
        )
...
def smart_commit(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        commit_hash = git.smart_commit(
            files=args.files, message=args.message or analysis.suggested_message
        )
...
def smart_branch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        branch_name = git.smart_branch(args.task, strategy=strategy_map[args.strategy])
...
def auto_pr(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        console.print(
            f'[dim]gh pr create --title "{pr_info["title"]}" --body "..."[/dim]'
        )
