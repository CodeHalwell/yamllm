...
def list_tools(args: argparse.Namespace) -> int:
    ...
    if hasattr(args, "category") and args.category:
        all_tools = {
            k: v for k, v in all_tools.items() if v.get("category") == args.category
        }
...
def test_tool(args: argparse.Namespace) -> int:
    ...
    console.print(
        "[dim]This will allow interactive testing of tool functionality[/dim]"
    )
    return 0
...
def manage_tools(args: argparse.Namespace) -> int:
    ...
    console.print(
        "[dim]This will allow enabling/disabling tools and managing configurations[/dim]"
    )
    return 0
...
def search_tools(args: argparse.Namespace) -> int:
    ...
    for tool_name, tool_info in all_tools.items():
        if (
            ...
            or query in tool_info.get("description", "").lower()
        ):
...
def setup_tools_commands(subparsers):
    ...
    # Tools list (default)
    tools_list_cmd = tools_sub.add_parser(
        "list", help="List available tools and tool packs"
    )
    ...
    # Tools info
    tools_info_cmd = tools_sub.add_parser(
        "info", help="Show detailed information about a tool"
    )
    ...
    # Tools manage
    tools_manage_cmd = tools_sub.add_parser(
        "manage", help="Interactive tool management"
    )
