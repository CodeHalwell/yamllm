...
class AsyncLLM:
    ...
    async def __aenter__(self):
        ...
        if self.provider_name.lower() == "openai":
            self.provider_client = AsyncOpenAIProvider(
                api_key=self.api_key, base_url=self.config.provider.base_url
            )
        ...
        if self.config.tools.enabled:
            self.tool_manager = AsyncToolManager(
                timeout=self.config.tools.tool_timeout, logger=self.logger
            )
    ...
    async def execute_with_tools(self, prompt: str, tools: List[str]) -> Dict[str, Any]:
        """
        ...
        return {"response": self._extract_text(response), "tool_results": []}
