...
class OpenAITemplate(ConfigTemplate):
    ...
    def generate_config(
        ...
        config = {
            ...
            "output": {"format": "text", "stream": streaming},
            "logging": {
                "level": "INFO",
                "file": "yamllm.log",
        ...
        config["embeddings"] = {"provider": None, "model": "text-embedding-3-small"}
...
class AnthropicTemplate(ConfigTemplate):
    ...
    def generate_config(
        ...
        openai_template = OpenAITemplate(
            "anthropic_base", "Base Anthropic config", "anthropic"
        )
...
class GoogleTemplate(ConfigTemplate):
    ...
    def generate_config(
        ...
        config["safety"]["content_filtering"] = (
            True  # Google has strong safety features
        )
...
class ConfigTemplateManager:
    ...
    def __init__(self):
        self.templates = {
            ...
            "anthropic": AnthropicTemplate(
                "anthropic", "Anthropic Claude models", "anthropic"
            ),
    ...
    def create_config(
        ...
        **kwargs,
        ...
        if provider not in self.templates:
            raise ValueError(
                f"Unknown provider: {provider}. Available: {list(self.templates.keys())}"
            )
    ...
    def validate_template_params(
        self, provider: str, preset: str, model: Optional[str] = None
    ) -> List[str]:
