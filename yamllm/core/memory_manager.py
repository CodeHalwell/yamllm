...
class MemoryManager:
    ...
    def __init__(
        ...
        self.vector_index_path = vector_index_path or os.path.join(
    ...
    def get_conversation_history(
        ...
        try:
            return self.conversation_store.get_messages(
                session_id=self.session_id, limit=limit or self.max_messages
            )
        except Exception as e:
    ...
    def add_vector(self, vector: List[float], message_id: int, content: str, role: str):
        ...
        try:
            self.vector_store.add_vector(
                vector=vector, message_id=message_id, content=content, role=role
            )
        except Exception as e:
            self.logger.error(f"Error adding vector to store: {e}")

    def search_similar(
        self, query_vector: List[float], k: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            return self.vector_store.search(
                query_vector=query_vector, k=k or self.vector_store_top_k
            )
        except Exception as e:
