...
class Usage(BaseModel):
    ...
    def calculate_total(cls, data: Any) -> Any:
        ...
        if isinstance(data, dict) and (
            "total_tokens" not in data or data["total_tokens"] == 0
        ):
    ...
    def adapt_generic_response(response: Any, provider: str) -> CompletionResponse:
        ...
        try:
            if isinstance(response, dict):
                ...
                return CompletionResponse(content=response.content, provider=provider)
            else:
                return CompletionResponse(content=str(response), provider=provider)
