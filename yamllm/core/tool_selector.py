...
class ToolSelector:
    ...
    def filter_tools_for_prompt(
        ...
        if explicit_tool:
            ...
            matching_tools = [
                t for t in tools if t.get("function", {}).get("name") == explicit_tool
            ]
            if matching_tools:
    ...
    def _extract_intent(self, prompt_text: str) -> Dict[str, bool]:
        ...
        command_match = re.search(
            r"(?:use|call|run|invoke)\s+(?:the\s+)?([a-z0-9_\-]+)\s+tool", text
        )
        ...
        wants = {
            ...
            "time": any(
                k in text for k in ("time in", "timezone", "utc", "pst", "est")
            ),
            "files": any(
                k in text
                for k in ("read file", "open file", "path", "search file", "grep")
            ),
            "url": bool(explicit_url) or bool(domain_hint),
            "csv": "csv" in text,
            "json": "json" in text
            and any(k in text for k in ("pretty", "minify", "validate")),
            "regex": "regex" in text
            or bool(re.search(r"\/[a-zA-Z0-9_\-\.\+\*\?\|\(\)\[\]\{\}]+\/", text)),
    ...
    def _extract_explicit_tool(self, prompt_text: str) -> Optional[str]:
        ...
        if not matches:
            ...
            bare_match = re.search(
                r"\buse\s+(webscrape|webscraper|web_scraper|webscraping)\b", text
            )
            if bare_match:
