...
def mask_string(
    sensitive_string: str, unmasked_prefix_len: int = 4, unmasked_suffix_len: int = 4
) -> str:
    """Masks a sensitive string, leaving a prefix and suffix unmasked."""
    if (
        not isinstance(sensitive_string, str)
        or len(sensitive_string) <= unmasked_prefix_len + unmasked_suffix_len
    ):
        return "********"
