...
class AsyncMistralProvider(AsyncBaseProvider):
    ...
    async def get_completion(
        ...
        **kwargs,
        ...
        if not self.client:
            raise ProviderError(
                "Mistral", "Client not initialized. Use async context manager."
            )
        ...
        try:
            ...
            if (
                ...
                result["tool_calls"] = self.format_tool_calls(
    ...
    async def create_embedding(
        self, text: str, model: str = "mistral-embed"
    ) -> List[float]:
        ...
        try:
            response = await self.client.embeddings(model=model, input=[text])
