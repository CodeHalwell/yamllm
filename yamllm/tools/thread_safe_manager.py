...
class ThreadSafeToolManager(ToolExecutor):
    ...
    def __init__(self, *, timeout: int = 30, max_concurrent: int = 5, logger=None):
        ...
        """
        super().__init__(
            timeout=timeout, logger=logger or logging.getLogger("yamllm.tools")
        )
        ...
        self._executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=max_concurrent, thread_name_prefix="tool_executor"
        )
    ...
    def register(self, tool) -> None:
        ...
        with self._lock:
            ...
            if (
                hasattr(tool, "requires_exclusive_access")
                and tool.requires_exclusive_access
            ):
    ...
    def execute(self, name: str, args: Dict[str, Any]) -> Any:
        ...
        if not self._execution_semaphore.acquire(timeout=self._timeout):
            raise ToolExecutionError(
                tool_name=name, message="Tool execution queue is full", tool_args=args
            )
        ...
        try:
            ...
            with self._execution_lock:
                if name in self._active_executions:
                    ...
                    msg = (
                        f"Tool '{name}' is already executing - possible recursive call"
                    )
    ...
    def execute_many(self, executions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
