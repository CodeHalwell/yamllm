...
class TerminalUI:
    ...
    def print_header(self, provider: str, model: str):
        ...
        if self.theme.layout.show_ascii_art:
            ...
            if art:
                self.console.print(
                    Panel(
                        Text(art, style=f"{self.theme.colors.primary} italic"),
                        box=box.ROUNDED,
        ...
        subtitle = Text("model: ", style="dim").append(
            model, style=self.theme.colors.secondary
        )
        self.console.print(
            Panel(
    ...
    def print_user_message(self, content: str):
        ...
        if self.style == "compact":
            self.console.print(
                Text("You ", style=f"bold {self.theme.colors.primary}").append(content)
            )
            return
        panel = Panel(
    ...
    def print_event(self, ev: dict):
        ...
        try:
            ...
            if et == "tool_request":
                ...
                panel = Panel(
                    Text(f"Model requested tools: {names}", style=color),
                    title="Tool Request",
                    border_style=color,
                    box=box.ROUNDED,
                )
                ...
                args = ev.get("args", {})
                preview = (
                    Text(json.dumps(args), style=color)
                    if args
                    else Text("(no args)", style=color)
                )
                panel = Panel(
                    preview,
                ...
                try:
                    rendered = (
                        ...
                        else Text(json.dumps(res, indent=2))
                    )
                except Exception:
                ...
                self.console.print(
                    Panel(info, title="Usage", border_style="grey39", box=box.MINIMAL)
                )
        except Exception:
            pass
    ...
    def stream_assistant_response(self, llm, prompt: str) -> str:
        ...
        with Live(
            self._assistant_panel(full_response),
            ...
            def on_delta(delta: str) -> None:
                ...
                while True:
                    ...
                    if match:
                        ...
                        live.update(
                            self._assistant_panel(Markdown(full_response.plain))
                        )
                    else:
                        break
