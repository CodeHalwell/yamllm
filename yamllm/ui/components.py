...
"""
    ...
    def print_config_summary(self, config: Dict[str, Any]):
        ...
        # Provider info
        table.add_row(
            "Provider", "Name", config.get("provider", {}).get("name", "Unknown")
        )
        table.add_row("", "Model", config.get("provider", {}).get("model", "Unknown"))
        ...
        if tools_enabled:
            ...
            table.add_row("", "Tools", f"{len(tools_list)} tools, {len(packs)} packs")

        # Memory
        memory_enabled = (
            config.get("context", {}).get("memory", {}).get("enabled", False)
        )
        table.add_row(
            "Memory",
            "Enabled",
    ...
    def create_tool_execution_panel(
        ...
        tree = Tree(
            f"[bold {self.theme['tool']}]🔧 {tool_name}[/bold {self.theme['tool']}]"
        )
    ...
    def print_tool_execution(
        ...
        panel = self.create_tool_execution_panel(
            tool_name, args, result, execution_time
        )
        self.console.print(panel)
...
class StreamingDisplay:
    ...
    def start(self, role: str = "assistant"):
        ...
        self.panel = self.console.create_message_panel(
            role=role, content="", timestamp=self.start_time
        )

        # Start live display
        self.live = Live(
            self.panel, console=self.console.console, refresh_per_second=10
        )
        self.live.start()
